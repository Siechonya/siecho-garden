---
{"dg-publish":true,"permalink":"/C++/2. C++面向对象编程/","noteIcon":"","created":"2025-08-24T23:01:59.987+08:00","updated":"2025-09-10T22:07:07.092+08:00"}
---
...
```
在参数列表(不论是函数声明还是定义)后加上 `const` 关键字(如 `Champion(int id, string nm, int hp, int mn, int dmg) const; `), 可以创建常量成员函数, 在该函数中不可修改本对象的成员变量.  
### 1.2.3 构造函数  
在上述代码中, 有一个<font color="#ff0000">和类同名的特殊成员函数</font> `Champion()`, 称为 [[C++/2. C++面向对象编程#1.3 Constructor (构造函数)\|Constructor (构造函数)]]. 其主要功能是初始化成员变量, 往往在类实例化(创建对象)时自动调用.  
...
```
### 1.2.5 this 指针  
在成员函数访问成员时, 有一个隐含的指针变量 `this` 可用, 它的类型是指向当前实例的指针, 指向正在调用该成员函数的对象.  
```C++
#include <iostream>
using namespace std;
...
class Product{
	...
	private:
		float price;
		static float discountRate;  // 静态成员变量
};
