---
{"dg-publish":true,"permalink":"/理力25秋/Answer for HW3/","noteIcon":"default","created":"2025-09-23T17:32:56.105+08:00","updated":"2025-09-28T08:16:23.622+08:00"}
---
...
$$
代入 Euler-Lagrange 方程 $\frac{d}{ds} \left( \frac{\partial L}{\partial \frac{ \mathrm{d} \mathbf{ r} }{ \mathrm{d} s}} \right) = \frac{\partial L}{\partial \mathbf{r}}$，可以得到
$$
...
\end{aligned}
$$
这称为光学角动量（守恒）。因为光线始终在一个平面传播，所以弧长微分满足：  
$$
...
yy' = 1+y'^2
$$
这正是悬链线方程，解为 $y = a \cosh \frac{x}{a}+b$。
# 3 修正最速降  
...
## 4.1 （a）  
选取参量如下图所示
![z_figure/Pasted image 20250926185631.png](/img/user/z_figure/Pasted%20image%2020250926185631.png)
那么拉格朗日量可以写作：  
$$
...
## 4.3 （c）  
把（4）代入（2）就得到（6），反解（4）得到（5）。接下来看为什么（5）和（6）描述圆内滚轮线。首先命名参量如下图：  
![z_figure/Pasted image 20250926194251.png](/img/user/z_figure/Pasted%20image%2020250926194251.png)
根据几何关系，有：  
$$
r^2 = (R-a)^2 + a^2 + 2(R-a)a\cos\varphi \overset{a=\frac{1}{2}(R-r_0)}= 
\frac{1}{2}(R^2 +r^2_0) + \frac{1}{2}(R^2 - r^2_0)\cos\varphi
$$
取 $\phi = \pi - \varphi$ 可以从上式得到（5）。以及再根据几何关系 
$$
\begin{aligned} 
a\varphi &= R\alpha \\
\frac{a}{\sin(\alpha - \theta)} &= \frac{{R-a}}{\sin(\varphi - (\alpha-\theta))}
\end{aligned}
\quad\Rightarrow\quad 
\tan\left( \theta - \frac{a\varphi}{R} \right) = -\frac{a\sin\varphi}{R+a(\cos\varphi - 1)}
$$
$$
\Rightarrow\quad
\theta = \frac{1}{2}\varphi - \frac{r_0}{2R}\varphi - \frac{(R-r_0)\sin\varphi}{(R+r_0)+(R-r_0)\cos\varphi}
$$
取 $\phi = \pi - \varphi$：   
$$
\theta = \frac{1}{2}\varphi - \frac{r_0}{2R}\varphi - \frac{(R-r_0)\sin\varphi}{(R+r_0)+(R-r_0)\cos\varphi}
$$
- [ ]  #todo 经过一通爆算 （助教还没爆算出来）：  
...
# 5 测地线  
## 5.1 （a）  
平面内的线元 $\mathrm{d} s=\left(\mathrm{d} x^{2}+\mathrm{d} y^{2}\right)^{\frac{1}{2}}=\sqrt{1+\dot{y}^{2}} \mathrm{~d} x$，则 $s=\int_{1}^{2} \sqrt{1+\dot{y}^{2}} \mathrm{~d} x$，对其求极值。令 $f=\sqrt{1+\dot{y}^{2}}$，则
$$
...
$$
代入欧拉方程 $\frac{\mathrm{d}}{\mathrm{d} x}\left(\frac{\partial f}{\partial \dot{y}}\right)-\frac{\partial f}{\partial y}=0$ 中，即 $\frac{\partial f}{\partial \dot{z}}=\frac{\dot{z}}{\sqrt{\dot{z}^{2}+R^{2}}}=  const.$，所以
$$
