---
{"dg-publish":true,"permalink":"/A_Space Physics/太阳物理/","noteIcon":"default","created":"2025-06-16T17:53:06.243+08:00","updated":"2026-03-25T11:51:34.281+08:00","dg-note-properties":{}}
---
...
> $$ L = 4\pi R_\odot^2 \sigma T_{\text{eff}}^4, \quad \sigma = 5.67 \times 10^{-8}\ \text{J}\cdot\text{m}^{-2}\cdot\text{s}^{-1}\cdot\text{K}^{-4} $$   
> $$ T_{\text{eff}} = 5770\ \text{K}, \quad L = 3.845 \times 10^{26}\ \text{J}\cdot\text{s}^{-1} $$
...
![zz_figure/Pasted image 20250618135121.png](/img/user/zz_figure/Pasted%20image%2020250618135121.png)  
- 较差自转的物理图像
![zz_figure/Pasted image 20250618141156.png](/img/user/zz_figure/Pasted%20image%2020250618141156.png)
## 2.2 太阳对流  
- 观测现象: (超)米粒组织
- 物理机制: 对流不稳定性
...
> [!info] 补充,
> 静力学平衡 $\frac{ d p }{ d r}=-\rho g$ 和 $p=\rho R T$ 时:
> $$ \left(\frac{dT}{dr}\right)_{ad} = \left(1 - \frac{1}{\gamma}\right) \frac{T}{p} \frac{dp}{dr} = -\frac{{\gamma - 1}}{\gamma R} g = - \frac{g}{c_p}$$
...
单位质量的流体元在位移后受到净浮力为：  
$$a_z = \frac{d^2 (\delta z)}{dt^2} = - \frac{g}{\rho} (\rho' - \rho)$$
环境密度在 $z$ 处为 $\rho(z)$。线性化：  
$$\frac{p(z)}{p_0} \approx 1 + \frac{d \ln p}{dz} \delta z, \quad \frac{\rho(z)}{\rho_0} \approx 1 + \frac{d \ln \rho}{dz} \delta z$$
流体元绝热变化，因此：   
$$
...
因此，Brunt-Väisälä 频率为：  
$$N_{BV} = \left[ g \left( \frac{1}{\gamma} \frac{d \ln p}{dz} - \frac{d \ln \rho}{dz} \right) \right]^{1/2}$$
...
![zz_figure/Pasted image 20250618160455.png](/img/user/zz_figure/Pasted%20image%2020250618160455.png)  
...
# 3 CH.3 太阳光球及其模型  
## 3.1 基本定义  
![zz_figure/Pasted image 20250618170004.png](/img/user/zz_figure/Pasted%20image%2020250618170004.png)
- 辐射强度:  
$$
...
$$
- 光球中的辐射转移方程
![zz_figure/Pasted image 20250618190030.png](/img/user/zz_figure/Pasted%20image%2020250618190030.png)
...
![zz_figure/Pasted image 20250618215721.png](/img/user/zz_figure/Pasted%20image%2020250618215721.png)
## 4.1 针状体和色球网络
...
![zz_figure/Pasted image 20250618215230.png](/img/user/zz_figure/Pasted%20image%2020250618215230.png)
上图中, 从线翼往线心, 辐射的来源高度越来越高, $K_1$ 形成于极小温度层, $K_2$ 形成于色球层温度上升区, $K_3$ 形成于更高层(密度很低, 已不满足“局部热动平衡”假设)。  
## 4.2 日冕辐射  
![zz_figure/Pasted image 20250618220914.png](/img/user/zz_figure/Pasted%20image%2020250618220914.png)
- 主要成分
	- 白光日冕(散射日冕)
...
等温理想气体满足状态方程(与等熵气体的 $\gamma p = c_s^2 \rho$ 不同)  
$$p = c_s^2 \rho, \quad c_s = \sqrt{\frac{k_B T}{\mu m_H}} \tag{5}$$
利用 (3) 和 (5) ，可以将 (4) 化为  
$$v_r \frac{\mathrm{d} v_r}{\mathrm{d} r} = \frac{GM}{r^2} - \frac{c_s^2}{\rho} \frac{\mathrm{d} \rho}{\mathrm{d} r} 
= -\frac{GM}{r^2} + {c_s^2}\left( \frac{2}{r} + \frac{1}{v_r} \frac{ \mathrm{d} v_r }{ \mathrm{d} r }  \right)
\tag{6}
$$
或  
$$\frac{ \mathrm{d} v_r }{ \mathrm{d} r}
= \frac{v_r}{r}\frac{{2c_s^2 - \frac{GM}{r}}}{v_r^2 - c_s^2}
\tag{7}
$$
方程 (7) 为非线性微分方程，有许多解族。当流速是声速 $c_s$ 时，方程(7)的右边发散，除非右边项的分子为零，即  
$$r_s = \frac{GM}{2c_s^2} \tag{8}$$
点 $(r_s, c_s)$ 称为临界声速点，其将方程(7)的解分为四个区域:  
![zz_figure/Pasted image 20250619203713.png](/img/user/zz_figure/Pasted%20image%2020250619203713.png)  

如果考虑磁场的影响, 临界声速点将变为临界阿尔芬速点, 但几个分区解的性质类似。
$(6)$ 式第一个等号积分有能量守恒  
$$\frac{v_r^2}{2} + c_s^2 \ln \left(\frac{\rho}{\rho_0}\right) - \frac{GM}{r} = E = const. \tag{9}$$
当解过临界点时, 在临界点 $(r_s, c_s)$ 有 $\rho_s = \frac {{r^2_0 v_0}} {r_s^2 c_s} \rho_0$, $r_s = \frac{GM}{2c_s^2}$,   
$$\frac{E_s(r_s, c_s)}{c_s^2} = -\frac{3}{2} + \ln \left[ \frac{v_0}{c_s} \left(\frac{r_0}{r_s}\right)^2 \right]\tag{10}$$
$(10)$ 式代入 $(9)$ 式, 取参考点 $(r_0, v_0)=(r_s, c_s)$, 得
$$\frac{1}{2} \left(\frac{v_r}{c_s}\right)^2 - \ln \left[ \left(\frac{r}{r_s}\right)^2 \frac{v_r}{c_s} \right] -2 \frac{r_s}{r} = -\frac{3}{2}  
\tag{11}$$
即  
$$v_r = c_s \left\{ \frac{4 r_s}{r} - 3 + 2 \ln \left[ \left(\frac{r}{r_s}\right)^2 \frac{v_r}{c_s} \right] \right\}^{1/2} 
\tag{12}$$
$$
...
$$
-  磁浮力不稳定性图示 ![zz_figure/Pasted image 20250619114505.png](/img/user/zz_figure/Pasted%20image%2020250619114505.png)  
- 衰减: 
	- 磁流管缩回到光球下面
	- 抛射到行星际空间
	- 磁能转化为热能和物质运动动能而耗散掉
...
- Hale极性定律(Hale 周期: 太阳磁场周期, 为 22 a)
	- Hale 极性定律图示 ![zz_figure/Pasted image 20250619121042.png](/img/user/zz_figure/Pasted%20image%2020250619121042.png)
...
![zz_figure/Pasted image 20250619133219.png](/img/user/zz_figure/Pasted%20image%2020250619133219.png)
注意日珥在上图轴线上, 即位于光球磁中性线上方.
##### 反常极性(K-R 模型)(二维) Kuperus-Roadu  
支撑日珥磁场的方向和由势场模型得到的方向相反  
![zz_figure/Pasted image 20250619133751.png](/img/user/zz_figure/Pasted%20image%2020250619133751.png)
##### 通量管模型(三维)  
![zz_figure/Pasted image 20250619134215.png](/img/user/zz_figure/Pasted%20image%2020250619134215.png)
##### 三维磁拱模型中性线上方的磁场是高度
...
##### 磁通量浮现模型  
单环耀斑图例:
![zz_figure/Pasted image 20250619162820.png](/img/user/zz_figure/Pasted%20image%2020250619162820.png)  
##### 倒 Y 型磁场模型  
双带耀斑图例:
![zz_figure/Pasted image 20250619163122.png](/img/user/zz_figure/Pasted%20image%2020250619163122.png)  
...
![zz_figure/Pasted image 20250619163322.png](/img/user/zz_figure/Pasted%20image%2020250619163322.png)
##### <font color="#ff0000">“标准模型”(CSHKP 模型)  (形成双带耀斑或耀斑爆发)</font>
![zz_figure/Pasted image 20250619161905.png](/img/user/zz_figure/Pasted%20image%2020250619161905.png)
...
| **辐射功率** | $P_{\perp} =\frac{e^{2} \mu_{0}}{6 \pi c} \frac{\gamma^{2}}{m^{2}}\left(\frac{d \vec{p}}{ d t}\right)^{2}, ~ \frac{d \vec{p}}{ d t} =e \vec{v} × \vec{B}$ | $P_{\perp}=\frac{\mu_{0} e^{2}}{6 \pi c} a_{\perp}^{2}$ | $P_{\text{brems}} = \frac{16\pi e^6 Z^2 n_e n_i}{3\sqrt{3}m_e^2 c^3} \cdot \sqrt{\frac{2\pi}{3kT_e/m_e}} \cdot \ln \Lambda$ , 此处 $\ln\Lambda$ 为库仑对数 |
##### 诱发辐射 (射电暴的辐射机制)
- 直接放大机制(电子回旋脉泽辐射机制)
...
### 5.6.1 典型三分量结构  
白光日冕仪中 CME 的典型三分量结构:
![zz_figure/Pasted image 20250619173346.png](/img/user/zz_figure/Pasted%20image%2020250619173346.png)  
...
![zz_figure/Pasted image 20250619175127.png](/img/user/zz_figure/Pasted%20image%2020250619175127.png)  
- 粒子加速
![zz_figure/Pasted image 20250619175220.png](/img/user/zz_figure/Pasted%20image%2020250619175220.png)
### 5.6.3 理想化的 CME 爆发伴随双带耀斑的<font color="#ff0000">物理模型</font>  
![zz_figure/Pasted image 20250619175923.png](/img/user/zz_figure/Pasted%20image%2020250619175923.png)
### 5.6.4 CME 的触发机制  
...
- 磁缰截断(Tether cutting): 强剪切磁拱在腿部的磁重联触发CME爆发。在AB和CD腿部交叉的“$\times$”处触发磁重联, 使暗条失去平衡而上升, 暗条上升拉出电流片。电流片的重联导致暗条的加速抛射及双带耀斑产生。
	![zz_figure/Pasted image 20250619195910.png](/img/user/zz_figure/Pasted%20image%2020250619195910.png)
##### 爆发成功的关键  
